...
export default function TeacherScreen(){
  ...
  const send=async(value=message)=>{
    ...
    try{
      const result=await learningApi.teacherChat(clean,'English','child');
      setReply(result.reply);
      setMessage('');
    }catch(e){
      setError(e instanceof Error?e.message:'Teacher AI no pudo responder.');
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.page} keyboardShouldPersistTaps="handled"><PremiumNav />
    <View style={styles.hero}><View style={styles.header}><View><Text style={styles.eyebrow}>TU PROFESOR DE IDIOMAS</Text><Text style={styles.title}>Teacher AI</Text><Text style={styles.subtitle}>Habla, escucha, pregunta y aprende a tu ritmo.</Text></View><View style={styles.status}><View style={[styles.dot,!error&&styles.dotOnline]}/><Text style={styles.statusText}>{error?'Revisar conexión':'Online'}</Text></View></View>
      <View style={styles.conversation}><View style={styles.avatarWrap}><Text style={styles.avatar}>👩🏻‍🏫</Text><View style={styles.aiBadge}><Text style={styles.aiBadgeText}>AI</Text></View></View><View style={styles.bubble}>{loading?<ActivityIndicator color="#7C49FF"/>:<Text style={styles.reply}>{reply}</Text>}</View></View>
      {error?<View style={styles.errorBox}><Text style={styles.errorTitle}>No pudimos conectar Teacher AI</Text><Text style={styles.errorText}>{error}</Text></View>:null}
    ...
    <View style={styles.composer}><TextInput value={message} onChangeText={setMessage} placeholder="Escribe qué quieres aprender..." placeholderTextColor="#7E90AB" style={styles.input} onSubmitEditing={()=>send()}/><Pressable style={[styles.send,loading&&styles.disabled]} disabled={loading} onPress={()=>send()}><Text style={styles.sendText}>{loading?'Pensando...':'Enviar ➜'}</Text></Pressable></View>
